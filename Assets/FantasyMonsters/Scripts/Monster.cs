...
namespace Assets.FantasyMonsters.Scripts
{
    ...
    public class Monster : MonoBehaviour
    {
        ...
        {
            OnEvent(eventName);
            if(enemy.typeRange == TypeRange.Melee)
            {
                enemy.SetDamage();
            }
            else if(enemy.typeRange == TypeRange.Shoot)
            {
                //enemy.SpawnBullet(anchorManager.Weapon.position);
            }
            else if(enemy.typeRange == TypeRange.Kamikaze)
            {
                enemy.Enemykamikaze();
            }
            Debug.Log("Attack");
        ...
        //Add
        public void AddEnemy(Enemy enemy_)
        {
            this.enemy = enemy_;
        }
